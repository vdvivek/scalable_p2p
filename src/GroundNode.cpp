...
GroundNode::GroundNode(NodeType::Type nodeType, std::string name, const std::string &ip, int port,
...
void GroundNode::updatePosition() {
  ...
  std::cout << NodeType::toString(this->type) << " " << this->name << "is stationary at "
            << "(" << this->coords.first << ", " << this->coords.second << ")" << std::endl;
}
...
void GroundNode::sendMessage(const std::string &targetName, const std::string &targetIP,
